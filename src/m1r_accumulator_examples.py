...
#                  return count
            ...
            #it set count to 0 in the start
            #math.cos should exist outside the loop
            #count is not counting cuz its set to 1 but before set to 0
            #it wants to include r and s and it currently is not
#
...
#              x = starting_point.x
#              for k in range(n):
#                  center = rg.Point(x + (k * diameter), y)            wrong one
#                  circle = rg.Circle(point, radius)
#                  circle.attach_to(window)
#
#              x = starting_point.x
